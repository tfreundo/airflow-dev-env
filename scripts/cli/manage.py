...
def manage_extension(extension_name):
    ...
    if answer == "1. Get the Repository":
        if extension_name == "MongoDB":
            subprocess.call(["cd", "../..", "&&", "git", "clone",
                             "https://github.com/tfreundo/airflow-dev-env-mongodb", mongodb_repo_foldername], shell=True)
        manage_extension(extension_name)
    elif answer == "2. Build the Container":
        if extension_name == "MongoDB":
            subprocess.call(["cd", "../../" + mongodb_repo_foldername, "&&",
                             "docker", "build", "--tag", mongodb_image_name, "."], shell=True)
        manage_extension(extension_name)
    elif answer == "3. Start the Container":
        if extension_name == "MongoDB":
            subprocess.call(["cd", "../../" + mongodb_repo_foldername, "&&", "docker", "run", "-d", "--publish",
                             "27017:27017", "--network", "airflow-dev-env", "--name", "mongo", mongodb_image_name], shell=True)
            print("### HINT ###")
...
def write_extrapackages(packages):
  ...
  if len(packages) > 0:
    lines_without_pipinstall.append(f"RUN pip install --no-cache-dir --user \"apache-airflow[{','.join(packages)}]\"")
